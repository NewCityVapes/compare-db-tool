...
async function fetchShopifyProducts() {
  ...
  while (hasNextPage) {
    ...
    const products = json.data.products.edges.map(({ node }) => {
      const price = parseFloat(node.variants?.edges[0]?.node?.price || "0.00");
      const ml = node.capacity_ml?.value
        ? parseInt(node.capacity_ml.value, 10)
        : null;
      const puffCount = node.puff_count?.value
      ...
      return {
        ...
        // Derived from price/puffCount/ml rather than separate metafields —
        // those had to be filled in by hand for every product and routinely
        // went stale or blank (e.g. showing $0.00/puff) whenever someone
        // forgot. Price, puff count and capacity already come from Shopify,
        // so there's nothing to type in twice.
        pricePerPuff: price && puffCount ? price / puffCount : null,
        pricePerML: price && ml ? price / ml : null,
        ...
        collectionHandle: node.collection_handle?.value || null,
      };
    });
    ...
    // A full 1s pause every 2 requests was pushing the whole sync past
    // Vercel's function timeout for catalogs of this size. Shopify's Admin
    // GraphQL API is cost-based (a refilling points bucket), not a strict
    // "N requests per second" limit, so a short pause on every request is
    // enough headroom for a one-time sync without needing the full second.
    if (hasNextPage) {
      await sleep(250);
    }
  }
